...
def cancel_order(query: str) -> dict:
    ...
    order_id = llm.with_structured_output(State).invoke(f'Extract order_id from the following text in json format: {query}')['order_id']
...
def check_inventory(state: MessagesState) -> MessagesState:
    ...
    item_id = llm.with_structured_output(State).invoke(f'Extract item_id from the following text in json format: {state}')['item_id']
    quantity = llm.with_structured_output(State).invoke(f'Extract quantity from the following text in json format: {state}')['quantity']
...
def compute_shipping(state: MessagesState) -> MessagesState:
    ...
    item_id = llm.with_structured_output(State).invoke(f'Extract item_id from the following text in json format: {state}')['item_id']
    quantity = llm.with_structured_output(State).invoke(f'Extract quantity from the following text in json format: {state}')['quantity']
    customer_id = llm.with_structured_output(State).invoke(f'Extract customer_id from the following text in json format: {state}')['customer_id']
...
def process_payment(state: State) -> State:
    ...
    cost = llm.with_structured_output(State).invoke(f'Extract cost from the following text in json format: {state}')
